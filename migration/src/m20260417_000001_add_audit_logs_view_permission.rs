...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        let mut insert_permission = Query::insert();
        insert_permission
            ...
            .values_panic([
                ...
                "view".into(),
                false.into(),
                "permissions.audit_logs.view.description".into(),
        ...
        let permission_ids = self
            .lookup_permission_ids(manager, "audit_logs", &["view"])
            .await?;
        if permission_ids.is_empty() {
            return Ok(());
        }

        let role_ids = self.lookup_role_ids(manager, &["Super Admin"]).await?;
        ...
        manager
            .exec_stmt(
                Query::delete()
                    ...
                    .and_where(Expr::col(Permissions::Domain).eq("audit_logs"))
                    .and_where(Expr::col(Permissions::Action).eq("view"))
                    .to_owned(),
            )
            .await?;
