...
fn find_first_semicolon(sql: &str) -> Option<usize> {
    let chars: Vec<char> = sql.chars().collect();
    let mut i = 0usize;
...
fn has_non_comment_tokens(sql: &str) -> bool {
    let chars: Vec<char> = sql.chars().collect();
    let mut i = 0usize;
...
pub fn is_read_only_sql(sql: &str) -> bool {
    ...
    if semicolons == 1 {
        if let Some(pos) = find_first_semicolon(trimmed) {
            if has_non_comment_tokens(&trimmed[pos + 1..]) {
                return false;
            }
        } else {
            return false;
