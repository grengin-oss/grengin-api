        ...
        for prompt in prompts {
            ...
            for file in &prompt.files {
                if file.content_type.starts_with("image/") {
                    // Handle images
                    if let Some(ref _file_id) = file.id {
                        // If we have a file ID, it's likely a URL or reference
                        // For now, skip files without actual data
                        // In production, you'd fetch the file data
                    }
                    // If file has base64 data embedded, add as image block
                    // This would require extending the File struct
                } else if file.content_type == "application/pdf" {
                    // Handle PDFs
                    // Similar to images, would need base64 data
                }
            }
