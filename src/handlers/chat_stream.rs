...
use crate::{
    ...
    services::{
        ...
        mcp_tools::{McpToolDescriptor, load_mcp_tools},
    ...
    services::{
        image_gen_helpers::{
            IMAGE_GENERATION_TOOL_NAME, generate_and_save, image_generation_tool_schema,
            resolve_image_tool_target,
        },
        models_cache::get_model_info_cached,
    },
    state::SharedState,
    utils::cost_calc::{
...
pub async fn handle_chat_stream(
    ...
    let (mut mcp_tool_lookup, mcp_server_summaries) = if supports_mcp_tools {
        load_mcp_tools(
            &app_state,
            claims.user_id,
    ...
    // Offered whenever the model can call tools at all: image_model_provider and
    // image_model pin which engine/model services it (either, both, or neither),
    // falling back to auto-select via defaultImageGenModel when neither is set.
    let image_tool_target = if supports_mcp_tools && !is_image_gen {
        resolve_image_tool_target(
            &app_state.database,
            req.image_model_provider.as_deref(),
            req.image_model.as_deref(),
        )
        .await
        .unwrap_or_else(|error| {
            eprintln!("image tool target resolution failed: {error:#}");
            None
        })
    } else {
        None
    };
    if image_tool_target.is_some() {
        mcp_tool_lookup.insert(
            IMAGE_GENERATION_TOOL_NAME.to_string(),
            McpToolDescriptor {
                openai_name: IMAGE_GENERATION_TOOL_NAME.to_string(),
                server_id: Uuid::nil(),
                server_name: "image-generation".to_string(),
                server_description: None,
                tool_id: Uuid::nil(),
                original_name: IMAGE_GENERATION_TOOL_NAME.to_string(),
                description: Some(
                    "Generate one or more images from a text prompt.".to_string(),
                ),
                input_schema: image_generation_tool_schema(),
                is_read_only: true,
    ...
    let sse_stream = async_stream::try_stream! {
       ...
       // Accumulated across tool rounds and re-included in every new_llm_message
       // metadata write below (there are 4), since each one rebuilds the whole
       // JSON object from scratch — omitting it from any single one would wipe
       // out images generated by an earlier round the moment that write lands.
       let mut generated_image_files: Vec<Value> = Vec::new();
       ...
       loop {
           ...
           while let Some(event) = tokio::select! {
               _ = cancel_handle.cancelled() => {
                   ...
                   new_llm_message.cost = Set(cancel_cost + image_gen_cost);
                   ...
                   new_llm_message.metadata = Set(Some(json!({
                       ...
                       "generatedImages": generated_image_files.clone(),
                   })));
                   new_llm_message
                       .clone()
                       .update(&app_state.database)
               ...
               Ok(LlmStreamEvent::Message { event: _, data }) => {
                   ...
                   match &parse_result {
                       ...
                       StreamParseResult::TokenUsage{ request_id:req_id,input_tokens, output_tokens, total_tokens:t_tokens, cached_input_tokens, cache_creation_tokens} => {
                          ...
                          new_llm_message.response_tokens = Set(response_tokens);
                          new_llm_message.total_tokens = Set(request_tokens + response_tokens);
                          new_llm_message.cost = Set(cost + image_gen_cost);
                          new_llm_message.metadata = Set(Some(json!({
                              "webSearch": req.web_search,
                              "cachedInputTokens": cached_input_tokens_acc,
                              "cacheCreationTokens": cache_creation_tokens_acc,
                     ...
                     LlmStreamError::Ended => {
                           ...
                           && tool_round < max_tool_rounds
                       {
                           ...
                           for call in pending_mcp_tool_calls.drain(..) {
                               ...
                               // Intercepted before any MCP-specific dispatch below: this
                               // tool has no real `server_id` (it's Uuid::nil()), so it must
                               // never reach the mcp_executions insert or client.call_tool
                               // lookup — both assume a genuine MCP server row and would
                               // either foreign-key-violate or 404.
                               if tool_ref.openai_name == IMAGE_GENERATION_TOOL_NAME {
                                   let Some((image_engine_key, image_model_name)) = image_tool_target.clone() else {
                                       continue;
                                   };
                                   let prompt = args_for_call
                                       .get("prompt")
                                       .and_then(Value::as_str)
                                       .map(str::to_string);
                                   let requested_count = args_for_call
                                       .get("count")
                                       .and_then(Value::as_u64)
                                       .map(|value| value.clamp(1, 4) as u8)
                                       .unwrap_or(1);

                                   let (output_payload, is_error) = 'image_tool: {
                                       let Some(prompt) = prompt else {
                                           break 'image_tool (
                                               json!({"error": "missing required argument: prompt", "is_error": true}),
                                               true,
                                           );
                                       };
                                       let image_provider = match resolve_provider(&app_state, &image_engine_key).await {
                                           Ok(provider) => provider,
                                           Err(error) => {
                                               break 'image_tool (
                                                   json!({"error": format!("image engine unavailable: {error}"), "is_error": true}),
                                                   true,
                                               );
                                           }
                                       };
                                       let image_model_info = get_model_info_cached(&app_state.req_client, &image_model_name)
                                           .await
                                           .ok()
                                           .flatten();
                                       let supports_multiple = image_model_info
                                           .as_ref()
                                           .map(|model| model.supports_multiple_images)
                                           .unwrap_or(false);
                                       let count = if supports_multiple { requested_count } else { 1 };
                                       ...
                                       {
                                           Ok(generated) => {
                                               let (total_text_input, total_image_input, total_output) = generated
                                                   .iter()
                                                   .fold((0i32, 0i32, 0i32), |acc, (_, _, ti, ii, o)| {
                                                       (acc.0 + ti, acc.1 + ii, acc.2 + o)
                                                   });
                                               let (img_input_rate, img_output_rate, img_image_input_rate, img_image_output_rate) =
                                                   image_model_info.as_ref().map_or((None, None, None, None), |model| {
                                                       (
                                                           model.input_token_rate,
                                                           model.output_token_rate,
                                                           model.image_input_token_rate,
                                               ...
                                               let effective_output_rate = if total_image_input > 0 {
                                                   img_image_output_rate.or(img_output_rate)
                                               } else {
                                                   img_output_rate
                                               };
                                               let call_cost = calculate_cost_decimal(total_text_input, 0, img_input_rate, None)
                                                   + calculate_cost_decimal(total_image_input, 0, img_image_input_rate, None)
                                                   + calculate_cost_decimal(0, total_output, None, effective_output_rate);
                                               image_gen_cost += call_cost;

                                               let mut file_ids = Vec::with_capacity(generated.len());
                                               for (file_id, content_type, _, _, _) in &generated {
                                                   let ext = if content_type == "image/png" { "png" } else { "webp" };
                                                   let file_meta = json!({
                                                       "message_id": new_message_id,
                                                       "file_id": file_id,
                                                       "content_type": content_type,
                                                       "cost": call_cost.to_f32().unwrap_or(0.0),
                                                   });
                                                   if let Ok(data) = serde_json::to_string(&file_meta) {
                                                       yield Event::default()
                                                           .event(ChatStreamEvents::ImageGenerated.to_string())
                                                           .data(data);
                                                   }
                                                   generated_image_files.push(json!({
                                                       "id": file_id,
                                                       "name": format!("{file_id}.{ext}"),
                                                       "content_type": content_type,
                                                   }));
                                           ...
                                           Err(error) => (
                                               json!({"error": format!("image generation failed: {error:#}"), "is_error": true}),
                                               true,
                                           ),
                                       }
                                   };
                                   ...
                                   new_llm_message
                                       ...
                                       .expect("failed to update tool results in new llm response in table messages");
                                   let chat_stream = ChatStream {
                                       id: None, title: None, message_id: None, is_new: None, content: None,
                                       input_tokens: None, output_tokens: None, latency_ms: None, cost: None,
                                       event: None, tool_call: None, cached_input_tokens: None,
                                       cache_creation_tokens: None, tool_result: Some(tool_result),
                                   };
                                   yield Event::default().event(ChatStreamEvents::ToolResult.to_string()).data(chat_stream.to_string());
           ...
           if stream_finished {
               ...
               if !artifact_accumulator.is_empty() {
                   ...
                   // Unconditional (was gated on saved_artifacts alone): a turn that only
                   // called the image tool, with no further model text afterward, would
                   // otherwise never persist generatedImages into this row at all — the
                   // client still got the image live via ImageGenerated above, but a later
                   // reload of the conversation would show none.
                   if !saved_artifacts.is_empty() || !generated_image_files.is_empty() {
                       new_llm_message.metadata = Set(Some(json!({
                           "webSearch": req.web_search,
