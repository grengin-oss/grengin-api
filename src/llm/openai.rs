use crate::{
    ...
    dto::{
        files::Attachment,
        llm::openai::{
            FileUploadResponse, OpenaiChatCompletionRequest, OpenaiChatCompletionResponse,
            OpenaiChatRequest, OpenaiEmbeddingRequest, OpenaiEmbeddingResponse, OpenaiInputItem,
            OpenaiListModelsResponse, OpenaiMessage, OpenaiModel, OpenaiTool, OpenaiToolChoice,
        },
    },
...
impl OpenaiHeaders for RequestBuilder {
    fn add_openai_headers(self, openai_sesstings: &OpenaiSettings) -> Self {
        ...
        if let Some(openai_org_id) = &openai_sesstings.org_id {
            req_builder = req_builder.header("OpenAI-Organization", openai_org_id);
        }
        if let Some(openai_project_id) = &openai_sesstings.project_id {
            req_builder = req_builder.header("OpenAI-Project", openai_project_id);
        }
        req_builder
    }
}
...
impl OpenaiApis for ReqwestClient {
    async fn openai_upload_file(
        ...
        let res = self
            .post(format!("{OPENAI_API_URL}/v1/files"))
    ...
    async fn openai_chat_stream_text(
        &self,
        openai_sesstings: &OpenaiSettings,
        model_name: String,
        temperature: Option<f32>,
        prompt: Vec<String>,
    ) -> Result<EventSource, Error> {
