...
const server = createServer((request, response) => {
  ...
  request.on('end', async () => {
    ...
    switch (`${request.method} ${url.pathname}`) {
      ...
      case 'POST /v1/models/mock-gemini-image:generateContent':
        return json(response, 200, {
          candidates: [{
            content: {
              parts: [
                { text: 'Generated by the mock.' },
                { inlineData: { data: Buffer.from('mock-gemini-png').toString('base64'), mimeType: 'image/png' } },
              ],
            },
          }],
