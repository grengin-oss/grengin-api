...
use crate::{
    ...
    ProviderEventErrorKind, RequestId, ResponseRule, TokenUsage, ToolCallId,
};
...
impl SseEventMapper {
    ...
    pub fn map(&mut self, event: &DecodedSseEvent) -> Result<Vec<ProviderEvent>, ProviderError> {
        if self.response.done_data.as_deref() == Some(event.data.trim()) {
            return Ok(self.complete(None));
        }

        let value = self.decode_data(event)?;

        let mut output = Vec::new();
        for rule in self.response.rules.clone() {
            if let Some(pointer) = &rule.for_each {
                ...
                for item in items {
                    if rule_matches(&rule, item) {
                        output.extend(self.map_rule(&rule, item)?);
                    }
                }
            } else if rule_matches(&rule, &value) {
                output.extend(self.map_rule(&rule, &value)?);
            }
        }
        Ok(output)
    }
    ...
    fn map_rule(
        ...
        let events = match rule.emit {
            EventKind::MessageStart => vec![ProviderEvent::MessageStart {
            ...
            EventKind::ToolCallStart => {
                ...
                if let Some(existing) = self.tool_ids_by_index.insert(index, id.clone()) {
                ...
                vec![ProviderEvent::ToolCallStart {
                    id,
                    name: required_string(rule, value, "name")?,
                    index,
                }]
            }
            EventKind::ToolArgumentsDelta => {
                let id = self.resolve_tool_id(rule, value)?;
    ...
    fn resolve_tool_id(
        ...
        rule: &ResponseRule,
        value: &Value,
    ) -> Result<ToolCallId, ProviderError> {
        if let Some(id) = optional_string(rule, value, "id")? {
            return Ok(ToolCallId::new(id));
        }
        let index = optional_u32(rule, value, "index")?.ok_or_else(|| {
            ProviderError::ResponseMapping(format!(
                "response rule {} requires a tool id or index",
                rule.id
            ))
        })?;
        self.tool_ids_by_index.get(&index).cloned().ok_or_else(|| {
            ProviderError::ResponseMapping(format!(
...
fn rule_matches(rule: &ResponseRule, value: &Value) -> bool {
    ...
    if let Some(not_null) = condition.not_null
        && (selected.is_some_and(Value::is_null) == not_null || (selected.is_none() && not_null))
    {
        return false;
    }
    true
...
mod tests {
    ...
    use crate::{
        ChatBodyEncoding, ChatResponseSpec, EventDataEncoding, EventKind, MatchCondition,
        ProviderEvent, ResponseRule,
    };
